  ...
  Widget build(BuildContext context) {
    return Consumer<SplashProvider>(
      builder: (context, splashProvider, _) {
        return !(splashProvider.configModel?.maintenanceMode?.maintenanceStatus == 1) && ((splashProvider.configModel?.whatsapp?.status ?? false) && splashProvider.configModel?.whatsapp?.number != null) ?
        InkWell(
          onTap: () async {
            final String? whatsapp = splashProvider.configModel!.whatsapp!.number;
            final Uri whatsappMobile = Uri.parse("https://api.whatsapp.com/send/?phone=$whatsapp");
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Text(getTranslated('chat', context)!, style: rubikSemiBold.copyWith(color: Colors.white),)
            ]),
          ),
        ) : const SizedBox();
      }
