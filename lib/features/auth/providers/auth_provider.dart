...
class AuthProvider with ChangeNotifier {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      if(canNotNavigate) {
        ...
        if(fromPage != null && fromPage == FromPage.profile.name){
          RouterHelper.getVerifyRoute(FromPage.profile.name, email, action: RouteAction.push);

        }else{
          RouterHelper.getVerifyRoute(FromPage.login.name, email, action: RouteAction.push);
      ...
      if(canNotNavigate) {
        ...
        if(fromPage != null && fromPage == FromPage.profile.name){
          RouterHelper.getVerifyRoute(
            FromPage.profile.name, phone, action: RouteAction.push,
          );
        }else{
          RouterHelper.getVerifyRoute(
            FromPage.login.name, phone, action: RouteAction.push,
          );
        }
      }
  ...
  Future<ResponseModel> checkPhoneForOtp(String phone) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      RouterHelper.getVerifyRoute(
        FromPage.otp.name, phone, action: RouteAction.push,
      );
    } else {
