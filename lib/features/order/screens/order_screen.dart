...
import 'package:acafe_customer/features/order/widgets/order_web_widget.dart';
...
import 'package:acafe_customer/features/order/widgets/order_list_widget.dart';
...
class _OrderScreenState extends State<OrderScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if(_isLoggedIn) {
      Provider.of<OrderProvider>(context, listen: false).getOrderList(context, orderFilter: "ongoing");
      Provider.of<OrderProvider>(context, listen: false).getOrderList(context, orderFilter: "history");
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: (ResponsiveHelper.isDesktop(context) ? const PreferredSize(preferredSize: Size.fromHeight(100), child: WebAppBarWidget())
          : CustomAppBarWidget(context: context, title: getTranslated('my_order', context), isBackButtonExist: !ResponsiveHelper.isMobile(),
        ...
        builder: (context, order, child) {
          return ResponsiveHelper.isDesktop(context) ? const OrderWebWidget() : Column(children: [
            ...
            Expanded(child: Center(child: SizedBox(width: Dimensions.webScreenWidth, child: Column(children: [
              ...
              Expanded(child: Consumer<OrderProvider>(builder: (ctx, orderProvider,_){
                return TabBarView(
                  controller: _tabController,
                  children: [
                    OrderListWidget(orderModel: orderProvider.ongoingOrder, filterType: "ongoing",),

                    OrderListWidget(orderModel: orderProvider.historyOrder, filterType: "history",),
                  ],
                );
              })),
            ])))),
