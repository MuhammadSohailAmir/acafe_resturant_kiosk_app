...
class ChatRepo {
  ...
  Future<ApiResponseModel> getAllConversationList(int offset, {String? search}) async {
    try {
      final response = await dioClient!.get('${AppConstants.allConversationList}${(search?.isNotEmpty ?? false) ? '?search=$search' : ''}');
      return ApiResponseModel.withSuccess(response);
